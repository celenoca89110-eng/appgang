(function () {
  ...
  function fmtDate(iso) {
    try {
      // PostgreSQL renvoie des timestamps deja au format ISO 8601 complet
      // (via la serialisation JSON d'Express), donc plus besoin de bidouille
      // de format comme avec les chaines datetime de SQLite.
  ...
  function renderSidebar() {
    ...
    list.innerHTML = state.gangs
      .map((g) => {
        ...
        if (canEdit() && editingGangId === g.id) {
          return `
        <div class="gang-card ${active} editing">
          <form class="inline-edit-form" data-edit-gang-form="${g.id}">
            <input type="text" name="gname" value="${escapeHtml(g.name)}" required maxlength="40" autofocus>
            <button type="submit" class="icon-btn subtle" title="Enregistrer">
            ...
            <button type="button" class="icon-btn" data-cancel-edit-gang="${g.id}" title="Annuler">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18"/><path d="M6 6l12 12"/></svg>
            </button>
          </form>
        </div>`;
        }
        ...
        <div class="gang-card ${active}" data-gang="${g.id}">
          ...
          ${canEdit() ? `<button class="icon-btn subtle" data-edit-gang-btn="${g.id}" title="Renommer ce gang">
  ...
  function renderCategoryBlock(gang, cat) {
    ...
    const catHeader =
      canEdit() && editingCategoryId === cat.id
        ? `<form class="inline-edit-form cat-inline-edit" data-edit-cat-form="${gang.id}|${cat.id}">
            <input type="text" name="cname" value="${escapeHtml(cat.name)}" required maxlength="30" autofocus>
            <button type="submit" class="icon-btn subtle" title="Enregistrer">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>
            </button>
            <button type="button" class="icon-btn" data-cancel-edit-cat="${cat.id}" title="Annuler">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18"/><path d="M6 6l12 12"/></svg>
            </button>
          </form>`
        : `<h3>${escapeHtml(cat.name)}</h3>
          <span class="category-count">${cat.items.length} item(s)</span>
          ${canEdit() ? `<button class="icon-btn subtle" data-edit-cat-btn="${cat.id}" title="Renommer cette catégorie">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
          </button>` : ''}
  ...
  document.body.addEventListener('click', async (e) => {
    ...
    const editGangBtn = e.target.closest('[data-edit-gang-btn]');
    const cancelEditGangBtn = e.target.closest('[data-cancel-edit-gang]');
    const editCatBtn = e.target.closest('[data-edit-cat-btn]');
    const cancelEditCatBtn = e.target.closest('[data-cancel-edit-cat]');
    ...
    try {
      ...
      if (editGangBtn) {
        e.stopPropagation();
        editingGangId = editGangBtn.getAttribute('data-edit-gang-btn');
        renderSidebar();
        return;
      }

      if (cancelEditGangBtn) {
        e.stopPropagation();
        editingGangId = null;
        renderSidebar();
        return;
      ...
      if (editCatBtn) {
        editingCategoryId = editCatBtn.getAttribute('data-edit-cat-btn');
        renderContent();
        return;
      }
  ...
  document.body.addEventListener('submit', async (e) => {
    ...
    const editGangForm = e.target.closest('[data-edit-gang-form]');
    const editCatForm = e.target.closest('[data-edit-cat-form]');

    try {
      if (editGangForm) {
        e.preventDefault();
        const gangId = editGangForm.getAttribute('data-edit-gang-form');
        const fd = new FormData(editGangForm);
        const name = (fd.get('gname') || '').toString().trim();
        if (!name) return;
        await Api.put(`/gangs/${gangId}`, { name });
        editingGangId = null;
        await loadGangs();
        render();
        return;
      ...
      if (editCatForm) {
        e.preventDefault();
        const [gangId, catId] = editCatForm.getAttribute('data-edit-cat-form').split('|');
        const fd = new FormData(editCatForm);
        const name = (fd.get('cname') || '').toString().trim();
        if (!name) return;
        await Api.put(`/gangs/${gangId}/categories/${catId}`, { name });
        editingCategoryId = null;
        await loadGangs();
        render();
        return;
